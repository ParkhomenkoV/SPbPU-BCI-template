...
\setlength{\belowcaptionskip}{-2pt} %расстояние между 
...
\captionsetup[table]{format=tablecaption,singlelinecheck=off,position=top,skip=0pt,labelfont={it,font13bp},textfont={bf,font13bp}}  % многострочные наименования и прочее
...
\newtheorem{sqnc}{Следствие}[chapter] % definition numbers are 
\newtheorem{propn}{Утверждение}[chapter] % definition numbers are dependent on theorem numbers
\newtheorem{lmm}{Лемма}[chapter]


\theoremstyle{mydefinition} %improved definition style
\newtheorem{exmp}{Пример}[chapter] % same for example numbers
\newtheorem{defn}{Определение}[chapter]  % definition numbers are 
